...
public class AlcoholController {
    ...
    @GetMapping("/search")
    public ResponseEntity<CursorPageResponse<AlcoholResponse>> search(
            @RequestParam @NotBlank(message = "검색어는 공백일 수 없습니다") @Size(min = 1, message = "검색어는 1자 이상 입력해주세요") String keyword,
            @RequestParam(required = false) String cursor,
            @RequestParam(defaultValue = "20") int size
    ) {
        return ResponseEntity.ok(alcoholService.search(keyword, cursor, size));
    }

    @Operation(summary = "카테고리별 술 목록", description = "카테고리로 술 목록을 커서 기반으로 조회합니다. 예: WHISKEY, WINE, BEER")
    @GetMapping
    public ResponseEntity<CursorPageResponse<AlcoholResponse>> getByCategory(
            @RequestParam AlcoholCategory category,
            @RequestParam(required = false) String cursor,
            @RequestParam(defaultValue = "20") int size
    ) {
        return ResponseEntity.ok(alcoholService.getByCategory(category, cursor, size));
    }
