...
### 기본 원칙 (2026-04-29 업데이트)
- **유저가 설계 먼저**: 기능 구현 시작 전, 유저가 먼저 설계 초안을 말한다. "엔티티는 이렇게, API는 이렇게, 삭제 순서는 이렇게 — 뭐가 문제야?" 형식으로 Claude에게 검토 요청.
- **Claude는 검토자**: Claude가 설계를 주도하지 않는다. 유저 초안의 문제점, 누락, 트레이드오프를 짚어주는 역할.
- **설명 못 하면 모르는 것**: 구현된 코드에 대해 유저가 본인 말로 설명할 수 있어야 한다. 설명 못 하면 Claude에게 설명 요청 후 이해하고 넘어간다.
- **이유 포함 필수**: 기능/방식 설명 시 성능, DB 부하, 유지보수성 등 근거를 항상 포함한다.

### 변경 배경
이전까지는 Claude가 설계를 주도하고 유저가 합의하는 방식이었음. 결과물은 나왔지만 유저가 "왜 이렇게 됐는지" 이해 없이 진행된 경우가 많았음. 앞으로는 유저가 먼저 생각하고 Claude가 보완하는 방식으로 전환.
...
3. 구현 전 토론 → 방향 합의
4. 코드 작성
5. 유저가 **"오늘은 여기까지"** 라고 하면 아래 두 가지 작업:
   - `context.md` 업데이트 (현재 브랜치, 구현 현황 반영)
   - 오늘 한 일 양식을 텍스트로 전달 (노션 붙여넣기용)

### 오늘 한 일 양식 (세션 종료 시 전달 형식)
```
# N회차 — YYYY-MM-DD | 제목
