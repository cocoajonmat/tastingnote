...
## @Modifying(clearAutomatically=true)와 JPA 엔티티 detachment

### 문제 발견 (14회차)

`updateNote()`에서 수정 내용이 PATCH 응답에는 반영되는데, 이후 조회 시 구버전 데이터가 나오는 버그 발견.

### 원인 분석

```
1. note.update(...)         → 엔티티 메모리 수정 (dirty 상태)
2. deleteAllByNoteId()      → @Modifying(clearAutomatically=true) 실행
                            → 영속성 컨텍스트(1차 캐시) 전체 초기화
                            → note가 detached 상태가 됨
3. 트랜잭션 커밋             → dirty checking 대상은 managed 엔티티만
                            → detached된 note는 대상 제외
                            → DB에 update 쿼리 미발생
```

### clearAutomatically=true가 필요한 이유

`@Modifying @Query`로 실행한 벌크 DELETE는 JPA를 거치지 않고 DB에 직접 쿼리를 날림.
이 때 1차 캐시에 남아있는 엔티티는 실제 DB 상태와 불일치 상태가 됨.
`clearAutomatically=true`는 이 불일치를 막기 위해 캐시를 초기화하는 옵션.
→ 필요한 설정이지만, 동시에 같은 트랜잭션 내 다른 엔티티도 함께 detach되는 부작용이 있음.

### 해결

```java
noteRepository.save(note);                        // 명시적으로 flush → DB에 먼저 기록
noteFlavorRepository.deleteAllByNoteId(noteId);   // 이후 캐시 초기화
```

`clearAutomatically=true` 실행 전에 `save()`로 명시적으로 flush하면,
캐시가 초기화되어 detach 되더라도 이미 DB에 기록된 상태라 문제없음.
