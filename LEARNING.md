...
JWT Stateless 구조와 충돌하므로, **클라이언트 사이드 방식**을 선택했다.

**플로우**:
1. 프론트 → 소셜 인가 URL 이동 → code 획득
2. `POST /api/auth/oauth/{provider} { code, redirectUri }` → 백엔드
3. 백엔드: code → access_token 교환 → 유저 정보 조회 → JWT 발급

각 소셜사마다 토큰 엔드포인트 URL과 응답 구조가 다르다:
- 카카오: `kakao_account.email`, `kakao_account.profile.nickname`
- 구글: flat 구조 (`id`, `email`, `name`, `picture`)
- 네이버: `response.id`, `response.email`, `response.nickname`

---

## List<OAuthClient>를 Map<Provider, OAuthClient>으로 변환 — 21회차

Spring은 같은 인터페이스를 구현한 Bean이 여러 개면 `List<OAuthClient>`로 주입해준다.  
생성자에서 이를 Map으로 변환하면 provider 문자열로 O(1) 조회가 가능하다.

```java
public OAuthService(List<OAuthClient> clients, ...) {
    this.clientMap = clients.stream()
        .collect(Collectors.toMap(OAuthClient::provider, Function.identity()));
}
```

S3Port와 동일한 인터페이스 분리 패턴 — 구현체를 교체해도 OAuthService는 변경 없음.

---

## Template Method 패턴 (AbstractOAuthClient) — 21회차

3개 OAuthClient 구현체의 HTTP 통신 로직이 완전히 동일했다(URL만 다름).  
추상 클래스가 공통 로직을 담고, 하위 클래스는 URL과 응답 파싱만 담당하는 구조.

```
OAuthClient (interface)
  └── AbstractOAuthClient (abstract, HTTP 통신 공통 로직)
        ├── KakaoOAuthClient  (URL + 카카오 응답 파싱)
        ├── GoogleOAuthClient (URL + 구글 응답 파싱)
        └── NaverOAuthClient  (URL + 네이버 응답 파싱)
```

인터페이스를 유지하면서 추상 클래스가 중간에 들어가는 구조라 확장성과 재사용성을 동시에 확보.  
새 provider 추가 시 `AbstractOAuthClient`를 상속해 URL 2개 + `parseUserInfo()`만 구현하면 됨.

---

## 커서 페이지네이션 설계 — 22회차

오프셋 방식(`LIMIT 20 OFFSET 40`)은 실시간 피드에서 데이터가 추가/삭제되면 중복/누락이 발생한다.  
커서 방식은 마지막으로 본 항목의 식별자(커서)를 기준으로 `WHERE id < :cursor` 조건으로 조회해 이를 방지한다.

**커서 인코딩**: `Map<String, String>` → `key=value&key=value` 형식 → Base64 URL-safe 인코딩  
- latest: `id=100`
- popular: `likeCount=42&id=100` (복합 정렬키)
- hot: `likeCount=42&createdAt=2026-01-01T12:00:00&id=100`

**sentinel 값**: 첫 페이지(cursor=null)일 때 `Long.MAX_VALUE`를 주입해 쿼리 조건을 단순화.  
`WHERE id < Long.MAX_VALUE` → 사실상 전체 조회.

**hasNext 판단**: size+1개를 조회해서 실제로 size+1개가 나오면 다음 페이지 있음 — DB 추가 쿼리 없이 O(1).

---

## Hot Score 2단계 조회 패턴 — 22회차

hot score = `likeCount / POWER(hoursAgo + 2, 1.5)` — Hacker News 방식.  
이 공식은 MySQL 전용(`TIMESTAMPDIFF`) native SQL이라 JPQL fetch join과 함께 쓸 수 없다.

**해결**: 2단계로 분리  
1. Native SQL → `SELECT n.id` (ID 목록만 조회, 정렬 유지)  
2. JPQL → `WHERE n.id IN :ids` + `JOIN FETCH` (엔티티 + 연관 데이터 일괄 로딩)  
3. Java에서 원래 ID 순서로 재정렬

**주의**: 2단계 사이에 `NOW()`가 다르므로 hot score 기준이 미세하게 달라질 수 있다 — 허용 가능한 수준.

---

## 이미지 N+1 방지 — findAllByNoteIdIn — 22회차

목록 조회에서 각 노트마다 `findAllByNoteId()` 호출 시 N번 쿼리 발생.  
`findAllByNoteIdIn(List<Long> noteIds)`로 한 번에 전체 이미지 조회 후 `groupingBy(noteId)`로 Map 구성.
...
`img.getNote().getId()`는 Hibernate 프록시 최적화로 DB 추가 조회 없이 FK 값만 반환된다 (ID 프록시 최적화).
...
**문제**: A가 `name="블레어우드 리저브"`로 PENDING 중일 때, B가 `nameKo="블레어우드 리저브"`로 요청하면 통과됨.  
name 체크는 `name` 컬럼만, nameKo 체크는 `nameKo` 컬럼만 보기 때문.

**해결**: name을 체크할 때 `existsByNameKoIgnoreCaseAndStatus(name, PENDING)`도 함께 체크.
nameKo를 체크할 때 `existsByNameIgnoreCaseAndStatus(nameKo, PENDING)`도 함께 체크.

→ 두 컬럼이 의미적으로 같은 데이터를 담을 수 있는 경우(영문명 ↔ 한글명 혼용 가능), 양방향으로 교차 검증해야 한다.

---

## JPQL에서 @ElementCollection 컬렉션 JOIN — 18회차

`@ElementCollection`으로 관리되는 컬렉션은 JPQL에서 직접 JOIN할 수 있다.

```java
@Query("SELECT COUNT(r) > 0 FROM AlcoholRequest r JOIN r.aliases a " +
       "WHERE LOWER(a) = LOWER(:alias) AND r.status = :status")
boolean existsByAliasIgnoreCaseAndStatus(@Param("alias") String alias,
                                          @Param("status") AlcoholRequestStatus status);
```

`r.aliases`가 `@ElementCollection List<String>`이면, JPQL에서 `JOIN r.aliases a`로 개별 요소를 `a`로 꺼낼 수 있다.  
일반 `@OneToMany` JOIN과 동일한 문법이지만 조인 대상이 엔티티가 아니라 값 타입 컬렉션이라는 점이 다르다.

---

## 에러코드 분리 원칙 — 같은 개념, 다른 원인 — 18회차

비슷해 보이지만 원인이 다른 오류는 에러코드를 분리해야 클라이언트가 원인을 구분할 수 있다.

**예시**: 술 등록 요청 중복 체크

| 상황 | 기존 에러코드 | 수정 후 에러코드 |
|------|------------|--------------|
| DB에 이미 등록된 술 | `DUPLICATE_ALCOHOL_REQUEST` | `ALCOHOL_ALREADY_EXISTS` (409) |
| 누군가 PENDING 중인 요청 | `DUPLICATE_ALCOHOL_REQUEST` | `DUPLICATE_ALCOHOL_REQUEST` (409) |

→ 클라이언트가 "이미 등록된 술이에요"와 "누군가 요청 중이에요"를 구분해서 다른 메시지를 보여줄 수 있다.  
에러코드는 HTTP 상태코드보다 더 세밀한 분류 체계 — 같은 409라도 의미가 다를 수 있다.

---

## 정책을 API 설계에 반영하기 — 18회차

"한 번에 하나의 별칭만 요청 가능" 정책을 구현할 때, DTO 타입 변경만으로 강제할 수 있다.

```java
// Before: List<String> aliases → 여러 개 한 번에 요청 가능 (정책 우회 가능)
// After: String alias → 단일 필드로 복수 요청 자체가 불가능
```

**원칙**: 비즈니스 규칙은 서비스 코드의 if 문보다 **타입 시스템**으로 먼저 표현하는 게 더 강력하다.  
타입 레벨 제약은 우회 자체가 불가능하지만, 런타임 체크는 항상 뚫릴 가능성이 있다.

---

## S3 삭제 실패 허용 전략 — 17회차 버그수정

S3 삭제와 DB 트랜잭션은 원자적이지 않다. 삭제 실패 시 두 가지 선택지:
- **삭제 실패 → 예외 전파**: 노트 삭제 자체를 막아버림 → 유저 입장에서 "삭제가 안 돼요" 상황 발생
- **삭제 실패 → 로그만 남기고 계속**: 고아 파일이 S3에 남지만 노트는 삭제됨

→ **후자가 올바른 설계**. 고아 파일은 S3 Lifecycle 정책(미참조 파일 자동 삭제)으로 주기적 정리 가능.
삭제 실패로 유저의 삭제 요청을 막는 것이 더 나쁜 UX.

---

## 방어적 프로그래밍 — 리소스 변경 전 유효성 선체크 — 17회차 버그수정

외부 시스템(S3, 결제 등)에 변경을 가하기 전에 애플리케이션 레벨 유효성 검사를 먼저 완료해야 한다.

```
❌ 잘못된 순서: S3 삭제 → 이미지 수 검증 → 예외 발생 → DB 롤백 (S3는 이미 삭제됨)
✅ 올바른 순서: 이미지 수 검증 → S3 삭제 → DB 저장
```

원칙: **되돌릴 수 없는 작업(외부 시스템 변경)은 항상 마지막에** 수행한다.

---

## MissingRequestHeaderException vs MissingServletRequestParameterException — 17회차 버그수정

Spring MVC에서 요청 데이터 누락 시 발생하는 예외가 다르다.

| 상황 | 예외 클래스 |
|------|------------|
| `@RequestParam` 누락 | `MissingServletRequestParameterException` |
| `@RequestHeader` 누락 | `MissingRequestHeaderException` |

둘 다 `GlobalExceptionHandler`에 별도로 등록해야 400을 반환한다. 핸들러 없으면 generic `Exception.class`로 빠져 500 반환.

---

## @RequestPart + Swagger Content-Type 문제 — 17회차

`@RequestPart`로 multipart JSON 파트를 받을 때 Swagger UI가 `application/octet-stream`으로 보내는 버그.

**원인**: Swagger UI는 `@RequestPart`의 복합 객체를 파일로 인식해서 Content-Type을 설정 안 함.

**해결**: 메서드 레벨에 `@io.swagger.v3.oas.annotations.parameters.RequestBody`에 `@Encoding(name = "note", contentType = "application/json")` 추가.
→ Swagger에게 "note 파트는 application/json으로 보내라"를 명시적으로 알려줌.

실제 curl 요청에서는 `-F 'note=...;type=application/json'`처럼 type을 붙여야 Spring이 JSON으로 파싱함.

---

## Port 패턴 (외부 시스템 추상화) — 17회차

외부 시스템(S3, Slack 등)에 의존하는 코드를 인터페이스로 감싸는 패턴.

```
S3Port (interface)
  └── S3Service (implements) → 실제 AWS SDK 호출
```

**왜 쓰나?**
- 테스트 시 Mock으로 교체 가능 (실제 S3 호출 없이 테스트)
- S3 → GCS 등 구현체를 바꿔도 서비스 코드 수정 없음
- 이 프로젝트에서 `NotificationPort`(Slack)와 동일한 패턴

---

## AWS DefaultCredentialsProvider — 17회차

AWS SDK v2에서 자격증명을 자동으로 찾아주는 provider.

찾는 순서: 환경변수 → Java 시스템 프로퍼티 → AWS 프로파일 파일 → EC2 Instance Metadata

로컬: `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY` 환경변수로 설정.
EC2/Lightsail: Instance Metadata(IAM Role) 또는 환경변수.
→ 코드에 자격증명을 하드코딩하지 않아도 돼서 보안상 안전.

---

## S3 버킷 정책 — 17회차

버킷 정책이 없으면 파일을 업로드해도 URL로 접근 시 403 에러.
...
`s3:GetObject`만 허용 → 누구나 읽기 가능, 쓰기/삭제는 불가.
이미지 URL을 브라우저에서 직접 열 수 있으려면 이 정책이 필수.
